...
def Receive():
    print("start Receive");
    cap = cv2.VideoCapture("rtsp://admin:admin1234@192.168.1.108/cam/realmonitor?channel=1&subtype=0");
    ret, frame = cap.read();
    q.put(frame);
    while ret:
        ret, frame = cap.read();
...
def Display(device, half, model, names, colors):
    print("start displaying");
    window_name = "192.168.1.108";
...
if __name__ == '__main__':
    ...
    p2 = threading.Thread(target = Display, args=(device, half, model, names, colors));
